  ...
  if (isComponentsEnabled) {
    if (fill->pre_script && strcmp(fill->pre_script, "begincomponent") == 0) {
      ...
      if (subrByGlyph.contains(compGlyph.charcode)) {
        ...
        if (subrByGlyphInfo.regionIndexesArrayIndex == -1 || mainGlyphregionIndexesArrayIndex == subrByGlyphInfo.regionIndexesArrayIndex) {
          *body = (*body)->next;
          contourLimits.setNext();
          while ((*body)->type != mp_stroked_code || ((mp_stroked_object*)*body)->pre_script == nullptr || strcmp(((mp_stroked_object*)*body)->pre_script, "endcomponent")) {
            *body = (*body)->next;
            contourLimits.setNext();
          }
          ...
          if (contourLimits.contours.size() > 0) {
            ...
            if (subrByGlyphInfo.pathlimits.currentx.deltas.size() > 0) {
              pathLimits.currentx = compPathLimits.currentx + subrByGlyphInfo.pathlimits.currentx;
              pathLimits.currenty = compPathLimits.currenty + subrByGlyphInfo.pathlimits.currenty;
            } else {
              pathLimits.currentx = compPathLimits.currentx + subrByGlyphInfo.lastx;
              pathLimits.currenty = compPathLimits.currenty + subrByGlyphInfo.lasty;
            }
          }
...
void ToOpenType::ContourLimits::setNext() {
