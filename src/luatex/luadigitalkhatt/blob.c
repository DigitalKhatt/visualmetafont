...
static int blob_new(lua_State *L) {
  ...
  luaL_getmetatable(L, "dkharfbuzz.Blob");
...
static int blob_get_length(lua_State *L) {
  Blob *b = (Blob *)luaL_checkudata(L, 1, "dkharfbuzz.Blob");
...
int dk_register_blob(lua_State *L) {
  return dk_register_class(L, "dkharfbuzz.Blob", blob_methods, blob_functions, NULL);
}
