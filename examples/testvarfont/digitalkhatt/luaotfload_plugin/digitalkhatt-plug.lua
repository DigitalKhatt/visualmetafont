...
local function shape_run(head, current, run, justify)
  if not run.skip then
    ...
    if glyphs then
      ...
      -- change font size
      if run.linewidth and digitalkhatt.justify and digitalkhatt.enableFontSizing then
        local fontid = run.font
        local fontdata = font.getfont(fontid)
        local space = fontdata.parameters.space
        local characters = fontdata.characters
        ...
        local nbpoints =  diff / 65536        
        if diff >  space then 
          
          local nbpoints =  diff / 65536

          local fontsize = fontdata.size / 65536

          local ratio = run.linewidth / currentlineWidth         -- 0.5                 
...
function process(head, fontid, _attr, direction,nofused, groupcode,linewidth,packtype)  
  ...
  cache = {}  
  local line
  local fieldwidth = getfield(head, "width") 
  local glue_order = getfield(head, "glue_order") 
  local glue_set = getfield(head, "glue_set") 
  local glue_sign = getfield(head, "glue_sign") 
  local testwidth = getwidth(head)
  local testid = getid(head)
  local tessubtype = getsubtype(head)
  if linewidth ~= 0 then      
      line = hpack(head,linewidth,packtype,direction)
      local testid = getid(line)
      local tessubtype = getsubtype(line)
...
add_to_callback('post_linebreak_filter', post_process_vlist_nodes, 'luaotfload.digitalkhatt.finalize_vlist')
add_to_callback('hpack_filter', post_process_nodes, 'luaotfload.digitalkhatt.finalize_hlist')
