...
luaotfload.digitalkhatt = harf_settings

digitalkhatt_settings.default_buf_flags = hb.Buffer.FLAGS_DEFAULT or 0
...
local function scalefont(data, spec)
  ...
  features.mode = 'plug'
  features.features = 'digitalkhatt'
  ...
  local tfmdata = {
    ...
    filename = 'digitalkhattloaded:' .. spec.resolved,
    ...
    hb = {
      ...
      hscale = upem,
      vscale = upem,
      buf_flags = digitalkhatt_settings.default_buf_flags,
    },
    specification = spec,
    shared = {
      features = features,
...
-- Register a reader for `digitalkhatt` mode (`mode=digitalkhatt` font option) so that we only
-- load fonts when explicitly requested. Fonts we load will be shaped by the
-- digitalkhatt plugin in luaotfload-digitalkhatt-plug.
...
luatexbase.add_to_callback('find_opentype_file', function(name)
  return find_file(name, 'opentype fonts')
      or name:gsub('^digitalkhattloaded:', '')
end, 'luaotfload.digitalkhatt.strip_prefix')
