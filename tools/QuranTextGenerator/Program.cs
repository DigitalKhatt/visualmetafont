...
using QuranTextGenerator;
using QuranTextGenerator.TarteelData;
...
namespace GenerateTexFromTanzil
{
  ...
  static class Program
  {
    ...
    static void Main(string[] args)
    {
      ...
      {
        QPCDoc.GenerateCPPFromUthmanicHafs(false, fileName);
        QPCDoc.GenerateCPPFromUthmanicHafs(true, fileName);
      }
      else if (arg == "tex")
      {
    ...
    {
      var pages = QPCDoc.readPages(fileName);
    ...
    static void compareHafsDocsPageByPage(string fileName1, string fileName2)
    {
      ...
      var pages1 = QPCDoc.readPages(fileName1);
      var pages2 = QPCDoc.readPages(fileName2);
      ...
      using (System.IO.StreamWriter file = new System.IO.StreamWriter(outputfile))
      {
        for (int nopage = 0; nopage < UthmanicHafsDocPages.Count; nopage++)
        {
          ...
          for (int i = 0; i < page.Count; i++)
          {
            ...
            string line = QPCDoc.replaceCharcatersFromUthmanicHafsDoc(page[i], nopage + 1, i + 1);
            if (nopage == 0 || nopage == 1)
            {
...
");
    ...
    static void GenerateXMLFromTanzil()
    {
      ...
      dynamic quran = JsonNode.Parse(quranText).AsObject();
    ...
    {
      generateXML(QPCDoc.readPages(fileName), outputFile);
    }
